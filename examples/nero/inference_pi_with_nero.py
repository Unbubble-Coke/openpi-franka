...
class Inference:
    def __init__(self, config_path: Path):
        ...
        requested_execute_horizon = run_cfg.get("execute_horizon", self.action_horizon)
        self.execute_horizon = max(1, min(int(requested_execute_horizon), int(self.action_horizon)))
        ...
        self.gripper_command_deadband = gripper.get("command_deadband", 0.0015)
        ...
        self._last_left_gripper_width: float | None = None
        self._last_right_gripper_width: float | None = None
    ...
    def get_obs_state(self) -> Dict[str, Any]:
        ...
        if self.robot_client:
            ...
            obs["left_gripper_position"] = l_grip_state
            ...
            obs["right_gripper_position"] = r_grip_state
        else:
    ...
    def _should_send_gripper_command(self, width: float, last_width: float | None) -> bool:
        return last_width is None or abs(width - last_width) >= self.gripper_command_deadband
    ...
    def _execute_delta_ee_actions(self, actions: np.ndarray):
        ...
        Action format (14D): [l_dx, l_dy, l_dz, l_drx, l_dry, l_drz,
                              ...
                              l_grip, r_grip]"""
        for action in actions[:self.execute_horizon]:
            ...
            if self._should_send_gripper_command(l_grip_width, self._last_left_gripper_width):
                self.robot_client.left_gripper_goto(width=l_grip_width, force=self.gripper_force)
                self._last_left_gripper_width = l_grip_width
            if self._should_send_gripper_command(r_grip_width, self._last_right_gripper_width):
                self.robot_client.right_gripper_goto(width=r_grip_width, force=self.gripper_force)
                self._last_right_gripper_width = r_grip_width
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                print(f"[ACTION] Raw inferred actions (execute_horizon={self.execute_horizon}):\n{result['actions'][:self.execute_horizon]}")
                logging.info(f"[ACTION] Raw inferred actions (execute_horizon={self.execute_horizon}):\n{result['actions'][:self.execute_horizon]}")
                if self.freeze_right_arm_action or self.freeze_right_gripper_action:
                    print(f"[ACTION] Executed actions after freeze mask:\n{exec_actions[:self.execute_horizon]}")
                    logging.info(f"[ACTION] Executed actions after freeze mask:\n{exec_actions[:self.execute_horizon]}")

                self.execute_actions(exec_actions)
                self.recorder.submit_actions(exec_actions[:self.execute_horizon], infer_time, obs.get("prompt", ""))
